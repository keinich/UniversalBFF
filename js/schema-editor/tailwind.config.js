...
let primaryColorHue = 225;
let lightDirection = 1;
let darkBordersDark = false;
let lightCurve = [
...
let ligthCurveWeight = 1.2;
let ligthCurveWeightDark = 0.25;
let ligthCurveWeightDark2 = 0.6;
let satCurve = [0, 1, 2, 3, 4, 5, 7, 9, 11, 13, 15, 18, 21, 24];
let satCurveDark = [0, 1, 2, 3, 4, 5, 7, 9, 11, 13, 15, 18, 21, 24];

function getBorder(i, d) {
  let light = getLight(i + 2, d);
  light = light + (d ? 10 : -5);
  sat = getSat(i, d);
...
function getSat(i, d) {
  if (d) {
    let maxSatStep = satCurveDark[satCurveDark.length - 1];
    let s = satDark - (satCurveDark[i] / maxSatStep) * satDark;
    return s;
  } else {
    let maxSatStep = satCurve[satCurve.length - 1];
    let s = sat - (satCurve[i] / maxSatStep) * sat;
    return s;
  }
}
...
function getPrimaryColor(i, d) {
  ...
  let primaryLight = 93 - i * 4;
  let primarySat = 100 - i * 6;
  return (
...
function getPrimaryColorDark(i) {
  let primaryLight = 25 + i * 5;
  let primarySat = 80 + i * 2;
  return (
    "hsl(" + primaryColorHue + "," + primarySat + "%," + primaryLight + "%)"
  );
}
